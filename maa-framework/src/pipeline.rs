...
mod tests {
    ...
    /// Locks the And/Or sub-recognition wire format produced by MaaFramework's
    /// PipelineDumper since 5.13.0-beta.6 (`{"sub_name", "recognition": {...}}`),
    /// alongside the node-name reference form.
    #[test]
    fn recognition_ref_parses_dumped_inline_and_node_name() {
        let json = r#"{
            "type": "And",
            "param": {
                "all_of": [
                    {"sub_name": "DirectHit", "recognition": {"type": "DirectHit", "param": {}}},
        ...
        let reco: Recognition = serde_json::from_str(json).expect("parse And");
        let Recognition::And(and) = reco else {
            panic!("expected And");
        };
        assert_eq!(and.all_of.len(), 3);
        assert_eq!(and.box_index, 1);
        match &and.all_of[0] {
            RecognitionRef::Inline(inline) => {
                assert_eq!(inline.sub_name.as_deref(), Some("DirectHit"));
        ...
        match &and.all_of[1] {
            RecognitionRef::Inline(inline) => {
                assert!(inline.sub_name.is_none());
                assert!(matches!(inline.recognition, Recognition::OCR(_)));
            }
            other => panic!("expected inline, got {other:?}"),
        }
        assert!(matches!(&and.all_of[2], RecognitionRef::NodeName(n) if n == "SomeNode"));

        // Round-trip keeps the nested form so the framework parser accepts it again.
        let out = serde_json::to_value(&and).unwrap();
        assert!(out["all_of"][0]["recognition"]["type"] == "DirectHit");
        assert!(out["all_of"][1].get("sub_name").is_none());
    }
}
